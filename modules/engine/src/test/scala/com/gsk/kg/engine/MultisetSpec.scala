  ...
  "Multiset.join" should {
    ...
    "join other Multiset[A] when they have both the same single binding" in {
      ...
      val df1 = List(
        ...
        ("test2", "graph1"),
        ("test3", "graph1")
      ).toTypedDF("d", GRAPH_VARIABLE.s).@@[Untyped]
      val df2 = List(
        ("test2", "graph1"),
        ("test3", "graph2"),
        ("test4", "graph2")
    ...
    "join other Multiset[A] when they share one binding" in {
      ...
      val ms1 = Multiset(
        ...
        List(
          ...
          .toTypedDF(d.s, e.s, GRAPH_VARIABLE.s)
          .@@[Untyped]
      )
      val ms2 = Multiset(
        ...
        List(
          ...
          .toTypedDF(d.s, f.s, GRAPH_VARIABLE.s)
          .@@[Untyped]
      )
      ...
      ms1.join(ms2) should equalsMultiset(
        Multiset(
          ...
          List(
            ...
            .toTypedDF("d", "e", "f", GRAPH_VARIABLE.s)
            .@@[Untyped]
        )
      )
    }
    ...
    "join other Multiset[A] when they share more than one binding" in {
      ...
      val ms1 = Multiset(
        ...
        ).toTypedDF(d.s, e.s, g.s, h.s, GRAPH_VARIABLE.s).@@[Untyped]
      )
      val ms2 = Multiset(
        Set(d, e, f, VARIABLE(GRAPH_VARIABLE.s)),
        List(
        ...
        ).toTypedDF(d.s, e.s, f.s, GRAPH_VARIABLE.s).@@[Untyped]
      )
      ...
      val expectedResult = Multiset(
        ...
        ).toTypedDF("d", "e", "g", "h", "f", GRAPH_VARIABLE.s).@@[Untyped]
      )
