...
class AbsSpec extends AnyWordSpec with Matchers with SparkSpec with TestConfig {
  ...
  "perform abs function correctly" when {
    "abs a valid double numeric" in {
      ...
      val expected = List(
        "\"1.65\"^^<http://www.w3.org/2001/XMLSchema#decimal>",
        "\"1.71\"^^<http://www.w3.org/2001/XMLSchema#decimal>",
        "\"1.4\"^^<http://www.w3.org/2001/XMLSchema#decimal>"
      ).map(Row(_))
    ...
    "term is a simple number" in {
      ...
      val expected =
        List(Row("\"10.4\"^^<http://www.w3.org/2001/XMLSchema#decimal>"))
